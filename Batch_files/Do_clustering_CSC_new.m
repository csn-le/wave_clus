...
handles.par.max_spk = 40000;                %max. # of spikes before starting templ. match (default 20000)
...
for k=1:length(channels)
    ...
    if nspk>15    
        ...
        if length(xf)>floor(sr*60)
            xf(floor(sr*60):end) = [];       %will plot just 60 sec.
        end
        plot((1:length(xf))/sr,xf(1:length(xf)))
        ...
        if print2file==0;
            ...
            eval(['print -djpeg ' handles.par.features '_fig2print_ch' num2str(channel)]);
        end
