function Do_clustering_pol(polytrodes)
...
handles.par.maxtemp = 0.301;                %maximum temperature
...
sr = 24414.1; % sampling frequency in Hz
...
for k=1:length(polytrodes)
    ...
    file_to_cluster = strcat('polytrode',num2str(polytrode),'_spikes.mat');
    filename = strcat('polytrode',num2str(polytrode));
    load(file_to_cluster)
    ...
    if print2file==0;
        ...
        eval(['print -djpeg40 fig2print_' filename ]);
    end

    %SAVE FILES
    par = handles.par;
    cluster_class = cluster;
    outfile=['times_' filename];
    if handles.par.permut == 'n'
